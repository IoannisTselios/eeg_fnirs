...
# === Load Data ===
noise_df = pd.read_csv("eeg_noise_detected_files.csv")

if os.path.getsize("eeg_clean_files.csv") > 0:
    clean_df = pd.read_csv("eeg_clean_files.csv")
else:
...
# === Plot 2: Channel × Frequency Heatmap ===
pivot = channel_df.pivot(index="Channel", columns="Frequency", values="Files > 100 pW")
plt.figure(figsize=(12, 10))
sns.heatmap(pivot, annot=True, fmt="d", cmap="viridis", linewidths=0.5)
plt.title("Files > 100 pW per Channel and Frequency")
plt.tight_layout()
plt.show()

# === Plot 3: Topomap per Frequency ===
threshold = 400  # pW
montage = make_standard_montage("brainproducts-RNP-BA-128")
